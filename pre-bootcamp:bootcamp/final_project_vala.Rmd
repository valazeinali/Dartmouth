...
design <- model.matrix(~0+as.factor(pheno$case[k])) #creating a design matrix and getting the independent variable ready for modeling 
...
direction <- sign(results$logFC) ## generates vector of numbers based on if the logFC has positve or negative momentum (1,-1)
esetSel <- eset[selected, ] #storing the occurences of <.001 into esetSel 
nrow(esetSel) # how many occurences of <.001
```

Okay! So we're now looking at just 175 probes!
...
heatmap(as.matrix(esetSel), col=topo.colors(100), ColSideColors=patientcolors, distfun = function(x) dist(x,method = 'euclidean'))
```
...
# create a vector of all genes in expr. Necessary for future steps to use gene stats to predict case outcome
...
plot.roc(pheno$case[-k]~newScore,data=data.frame(cbind(pheno$case[-k],newScore)),add=T,print.auc=T, ci=T, col="red", print.auc.y=0.4)
...
dp <- ggplot(df, aes(x=as.factor(model1$model$mergeCase), y=model1$model$allScore, fill=as.factor(model1$model$mergeCase))) +
  geom_violin(trim=FALSE)+
  geom_boxplot(width=0.1, fill="white")+
  labs(title="Plot of case by score",x="Case ", y = "Score")+
  stat_compare_means(label.x = 1.5, label.y = 1, size=10)+
...
## Enter your own code here
plot.roc(model1$model$mergeCase ~ model1$model$allScore, data=df, legacyaxes=F,print.auc=T, ci=T, main="AUC for Diagnostic Score")
plot.roc(pheno$case[-k] ~ newMScore, data=data.frame(cbind(pheno$case[-k],newMScore)), add=T,print.auc=T, ci=T, col="red", print.auc.y=0.4)
legend("bottomright",c("Training Data","Test Data"),lty=c(1,1),col=c("black","red"))
```
